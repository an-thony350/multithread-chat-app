...
static int sockaddrs_equal(const struct sockaddr_in *a,
...
static ClientNode *add_client(const char *name,
...
static int recipient_has_muted_sender(ClientNode *recipient,
...
// Note: both fns trim_spaces and rtrim could be combined, but i kept separate
// for clarity
  ...
  *d = '\0';
  ...
  if (strcmp(type, "conn") == 0) {
    ...
    if (existing) {
      if (find_client_by_name(payload) &&
          strcmp(existing->client.name, payload) != 0) {
        pthread_rwlock_unlock(&clients_lock);
        char resp[BUFFER_SIZE];
    ...
    snprintf(welcome, BUFFER_SIZE,
             "SYS$Hi %s, you have successfully connected to the chat\n",
             payload);
    ...
    broadcast_all(sd, announce,
                  sender); // skip sender (sender already got welcome)
  ...
  if (strcmp(type, "sayto") == 0) {
    ...
    if (!recipient_name || !msg_rest) {
      char resp[BUFFER_SIZE];
      snprintf(resp, BUFFER_SIZE,
               "ERR$sayto requires a recipient and message\n");
      udp_socket_write(sd, &client_addr, resp, strlen(resp));
      return NULL;
    }
  ...
  if (strcmp(type, "mute") == 0) {
    if (!sender) {
      char resp[BUFFER_SIZE];
      snprintf(resp, BUFFER_SIZE,
               "ERR$You must conn$<name> before muting users\n");
      udp_socket_write(sd, &client_addr, resp, strlen(resp));
      return NULL;
    }
    if (payload == NULL || strlen(payload) == 0) {
  ...
  if (strcmp(type, "unmute") == 0) {
    if (!sender) {
      char resp[BUFFER_SIZE];
      snprintf(resp, BUFFER_SIZE,
               "ERR$You must conn$<name> before unmuting users\n");
      udp_socket_write(sd, &client_addr, resp, strlen(resp));
      return NULL;
    }
    if (payload == NULL || strlen(payload) == 0) {
