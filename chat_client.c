...
void *listener_thread(void *arg) {
  ...
  while (1) {
    ...
    if (rc > 0) {
      ...
      pnoutrefresh(chat_pad,
                   (chat_lines > rows - 2 ? chat_lines - (rows - 2) : 0), 0, 0,
                   0, rows - 3, cols - 1);
...
/*
  ...
  Prints a message sent by the user to the chat pad with timestamp. (Same
  structure as listener thread)
...
static void print_own_message(const char *msg) {
  ...
  pnoutrefresh(chat_pad, (chat_lines > rows - 2 ? chat_lines - (rows - 2) : 0),
               0, 0, 0, rows - 3, cols - 1);
...
void redraw_pad() {
  ...
  pnoutrefresh(chat_pad, view_top, 0, 0, 0, rows - 3, cols - 1);
  doupdate();
  pthread_mutex_unlock(&ui_lock);
}
